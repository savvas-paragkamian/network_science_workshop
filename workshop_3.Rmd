...
In large scale analyses genes and/or proteins are used as statistical objects. This approach can be enriched with existing knowledge for each gene-protein in order to obtain more insightful and biologicaly relevant results. This is a way of combining knowledge from single gene/protein genetic and biochemical research with the holistic approach of big data.
...
proteins_cluster <- intresting_genes_Essential
myEID <- unlist(mget(proteins_cluster, org.Dm.egFLYBASE2EG, ifnotfound=NA))
myEID <- as.character(myEID)

kegg <- as.data.frame(unlist(mget(as.character(myEID), org.Dm.egPATH,
       ifnotfound=list(NA))),row.names = NULL) %>% tibble::rownames_to_column(.,var="Var1")
colnames(kegg) <- c("entrez_id", "kegg_pathway_id")

kegg_summary <- kegg %>% group_by(kegg_pathway_id) %>% summarize(proteins=n()) %>% ungroup()

kegg_summary[1,3] <- keggGet("dme00190")[[1]]$NAME
kegg_summary[2,3] <- keggGet("dme01100")[[1]]$NAME
kegg_summary[3,3] <- keggGet("dme03010")[[1]]$NAME
kegg_summary[4,3] <- keggGet("dme03050")[[1]]$NAME

colnames(kegg_summary) <- c("kegg pathway id", "proteins", "kegg pathway name")

write.table(x = kegg_summary, file = "../Thesis_Essentiality_Drosophila_Signed_Network/Tables/Essential_cluster_kegg_summary.csv", sep = ",", col.names = TRUE,row.names = FALSE)

library(pathview)

pv.out_1 <- pathview(gene.data = kegg$entrez_id, pathway.id = as.character(kegg_summary$kegg_pathway_id[1]), species = "dme", out.suffix = "dros_essential_cluster", kegg.native = F)

pv.out_2 <- pathview(gene.data = kegg$entrez_id, pathway.id = as.character(kegg_summary$kegg_pathway_id[2]), species = "dme", out.suffix = "dros_essential_cluster", kegg.native = F)

pv.out_3 <- pathview(gene.data = kegg$entrez_id, pathway.id = as.character(kegg_summary$kegg_pathway_id[3]), species = "dme", out.suffix = "dros_essential_cluster", kegg.native = F)

pv.out_4 <- pathview(gene.data = kegg$entrez_id, pathway.id = as.character(kegg_summary$kegg_pathway_id[4]), species = "dme", out.suffix = "dros_essential_cluster", kegg.native = F)

pv.out_4b <- pathview(gene.data = kegg$entrez_id, pathway.id = as.character(kegg_summary$kegg_pathway_id[4]), species = "dme", out.suffix = "dros_essential_cluster", kegg.native = F,same.layer = F)



## 1

retrieveKGML("dme00190", organism="dme",destfile = "../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme00190kegg.xml", method="wget", quiet=TRUE)

dme00190pathway <- parseKGML("../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme00190kegg.xml")

dme00190pathway2 <- KEGGpathway2Graph(dme00190pathway, expandGenes=TRUE,genesOnly = T)


## 2

# retrieveKGML("dme01100", organism="dme",destfile = "../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme01100kegg.xml", method="wget", quiet=TRUE)
# 
# dme01100pathway <- parseKGML("../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme01100kegg.xml")
# 
# dme01100pathway2 <- KEGGpathway2Graph(dme01100pathway, expandGenes=TRUE,genesOnly = T)
# dme01100pathway2

## 3

retrieveKGML("dme03010", organism="dme",destfile = "../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme03010kegg.xml", method="wget", quiet=TRUE)

dme03010pathway <- parseKGML("../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme03010kegg.xml")

dme03010pathway2 <- KEGGpathway2Graph(dme03010pathway, expandGenes=TRUE,genesOnly = T)


## 4
retrieveKGML("dme03050", organism="dme",destfile = "../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme03050kegg.xml", method="wget", quiet=TRUE)

dme03050pathway <- parseKGML("../Thesis_Essentiality_Drosophila_Signed_Network/Data/dme03050kegg.xml")

dme03050pathway2 <- KEGGpathway2Graph(dme03050pathway, expandGenes=TRUE,genesOnly = F)


# png <- keggGet(c( "dme01100"), "image") 
# t <- tempfile()
# library(png)
# writePNG(png, t)
...
# pId <- as.character(kegg_summary$kegg_pathway_id[2])
# retrieveKGML(pId, organism="dme", destfile=tmp, method="wget", quiet=TRUE)
